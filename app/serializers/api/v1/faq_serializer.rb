...
class Api::V1::FaqSerializer < Api::V1::BaseSerializer
  ...
  def load_buttons
    @buttons = Button.by_ids(object.faqs_resourceables.by_resourceable_type(Button.name).pluck(:resourceable_id))
  end

  def load_answers
    @answers = Answer.by_ids(object.faqs_resourceables.by_resourceable_type(Answer.name).pluck(:resourceable_id))
  end
end
